...
- **Submission Artifact Size (SUPERSEDED)**: `189,846,528 bytes` [SUPERSEDED — containerd snapshotter export format].
...
## 5. Artifact Size Resolution & Gzip Round-Trip Verification (30-08-2026 — Task P9)

- **True `image.tar` Size**: **`574,480,384 bytes`** (`564 MB`, standard Docker uncompressed tar archive export from overlay2 driver).
- **True `image.tar.gz` Size**: **`186,993,701 bytes`** (`187 MB`), compressed with Python `gzip` (level 6).
- **Compression Ratio**: **`32.55%`** (`186,993,701 / 574,480,384`), confirming that the previously quoted ~189.8 MB metric corresponds to compressed layer format.
- **Round-Trip Load Verification**: Decompressing `image.tar.gz` back to `image_roundtrip.tar` yielded exactly **`574,480,384 bytes`** (byte-for-byte match), and `docker load -i image_roundtrip.tar` loaded image ID **`447f647fbc9f`** identically.
- **Source-to-Artifact Parity**: `git diff --stat bbf68d7..HEAD -- src/ models/ config.yaml Dockerfile run.sh requirements.txt` is **EMPTY** (0 files changed across all shipped artifacts; `bbf68d7..HEAD` commits are exclusively documentation).
